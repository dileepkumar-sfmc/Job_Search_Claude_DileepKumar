...
- Line 1 = full name; line 2 = **professional title/headline** (`<Role> | <focus>`, adapted to the target job's role but only when the resume genuinely supports it — otherwise the candidate's real title); line 3 = pipe-separated contact (`phone | email | location | linkedin`)
...
- The renderer keys off: line index (name = line 0; a pipe-separated line carrying an email/URL/phone = contact; line 1 otherwise = the title/headline), the `SECTION_HEADINGS` set, `^[-•*] ` bullets, and `^Label: value` lines (`Client` gets right-aligned dates via a right tab stop; `Environment` is italic). It must stay ATS-safe — single column, no tables. Output is tailored/reordered per JD but facts (companies, titles, dates) stay truthful.
