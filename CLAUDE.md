...
**Two sources, chosen by whether a jobs-API key is set.** The modal prefers **real postings** when `jobsApiKey` is configured (Settings → Job Search), and falls back to **AI leads** otherwise. `useRealApi = Boolean(jobsApiKey)` in `JobSearchModal.tsx` drives the branch; `source` state (`'api' | 'ai'`) tweaks the labels.

- **Real postings (preferred):** `searchRealJobs()` in `lib/jobsApi.ts` calls **JSearch on RapidAPI** (`jsearch.p.rapidapi.com/search`) — aggregates Google for Jobs, so every result is an actual open listing with a working `job_apply_link`. Nothing is synthesized. It maps prefs → JSearch params (`datePosted`→`date_posted` all/today/3days/week/month; employment types→`employment_types` FULLTIME/CONTRACTOR/PARTTIME/INTERN; `workMode==='remote'`→`remote_jobs_only`; role+seniority+location folded into the free-text `query`) and maps the response → `JobSearchResult` (real `url`, relative `posted` from `job_posted_at_datetime_utc`, `fit: ''`). The key is **`jobsApiKey`** in `store/settings.ts` — **local-only (never synced), dev-auto-loads from `VITE_JOBS_API_KEY` gated behind `import.meta.env.DEV`** exactly like the AI key. In the result list, `source==='api'` leads with an **Apply ↗** (the real link) + a secondary "Search more".
