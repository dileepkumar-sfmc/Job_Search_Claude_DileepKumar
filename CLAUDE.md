...
`GeneratedDocs.tsx` downloads each tab: resume + cover letter → `.docx` (resume uses `kind: 'resume'`, the structured renderer; cover letter uses the plain prose renderer), interview prep + company brief + both email tabs (recruiter reply + application email) → `.txt`. Saving prefers the File System Access API (`showSaveFilePicker` — native Save-As dialog) and falls back to `file-saver`; the picker is blocked in cross-origin iframes (e.g. the embedded preview pane) so it silently downloads to the Downloads folder there.
