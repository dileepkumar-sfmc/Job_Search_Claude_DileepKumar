...
- **Generate All** → `generateAll()` in `lib/ai.ts`: one prompt, expects a JSON object with all five keys (`coverLetter`, `tailoredResume`, `interviewQuestions`, `companyBrief`, `outreachEmail`). One API call — cheapest when you want everything.
...
The fifth doc, **`outreachEmail`** (`EMAIL_SPEC`), drafts a ready-to-send recruiter **reply** in a fixed structure (matches the user's real template): "Thank you for reaching out…" → a numbered **details** block (Current Location, **Work Authorization: H1B – No sponsorship required**, **Expected Rate: market rate on C2C**, Reason for Change, Best Fit) → a **"Skill-wise details as requested:"** numbered list mapping the JD's key skills to the candidate's real experience → "Best time to connect… Best number: **+18038146712**" → a signature (name, phone, email, linkedin). Baked-in stance (H1B/no-sponsorship/C2C/phone) lives in `EMAIL_SPEC` — edit there to change it. Edit `EMAIL_SPEC` to change that stance. It renders as the **Email** tab (`.txt` download) and adds an **"Open in Gmail"** button: `splitEmail()` parses the `Subject:` line vs. body, and `JobDetailDrawer` extracts a `recruiterEmail` from the job text (`rawText`), so the button opens a Gmail compose deep-link (`mail.google.com/mail/?view=cm&su=&body=&to=`) pre-filled with native Gmail formatting — no copy-paste.
...
- **Hard-locked fields (overridden in the prompt, NOT taken from the source resume):** the contact-line **phone is always `+18038146712`**, and the **Education section is always fixed** to `Master of Science in Computer Science — University of the Cumberlands, 2016` and `Bachelor of Science in Computer Science — Jawaharlal Nehru Technological University (JNTUH), 2013`. To change them for a different person/credentials, edit `RESUME_SPEC` in `lib/ai.ts` (Line 2 contact instruction + Education instruction) — it feeds both `generateAll` and `generateDoc`. Everything else (name, email, location, linkedin, experience, skills) still comes from the parsed resume.

`GeneratedDocs.tsx` downloads each tab: resume + cover letter → `.docx` (resume uses `kind: 'resume'`, the structured renderer; cover letter uses the plain prose renderer), interview prep + company brief + outreach email → `.txt`. Saving prefers the File System Access API (`showSaveFilePicker` — native Save-As dialog) and falls back to `file-saver`; the picker is blocked in cross-origin iframes (e.g. the embedded preview pane) so it silently downloads to the Downloads folder there.
